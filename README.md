...
```shell
hostname && hostname -I (or maybe hostname -i, not sure...)
```
Note the IP address and the hostname that corresponds with it. You will need this when we configure Caddy later.
5. Run the following (**on the VM**):
```shell
...
6. Go ahead and run through steps 1-5 again in order to set up your second VM and Nextcloud AIO instance.
7. Almost done! All that's left is configuring Caddy (**on the physical host machine**):
```shell
