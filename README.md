...
In order to create VMs, we'll need a few tools. Assuming your physcial host is a barebones Linux server without a desktop environment installed, the easiest way to create VMs is to install QEMU/KVM + virsh + virt-install ([more info](https://wiki.debian.org/KVM)). To do so on Debian, run this command (**on the physical host machine**):
```shell
