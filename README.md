...
1. That's it! Now, all that's left is to set up your instances through the AIO interface as usual by visiting `https://example1.com:8443` and `https://example2.com:8443` in a browser. Once you're finished going through each setup, you can access your new instances simply through their domain names. You can host as many instances with as many domain names as you want this way, as long as you have enough system resources. Enjoy!
    ...
    <details><summary>A few extra tips for managing this setup!</summary>
        <ul>
            ...
            <li>You might want GPT if your instance ends up using 2TB or more storage. The Debian installer only utilizes GPT in certain situations. To use GPT instead of MBR, we can start the installer in UEFI mode. Run (<strong>on the physical host machine</strong>): <pre>apt install ovmf</pre> Then, adjust your virt-install command from above in step 1 by adding the following flag: <pre>--boot uefi</pre> Finally, confirm the use of GPT using: <pre>parted -l # Install parted (apt install parted), then make sure it returns "gpt" and not "msdos"</pre></li>
